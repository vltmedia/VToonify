...
class TestOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--content", type=str, default='./data/Justin.jpg', help="path of the content image/video")
        ...
        self.parser.add_argument("--ckpt", type=str, default='./checkpoint/vtoonify_d_isekai/vtoonify_s_d_c.pt', help="path of the saved model")
...
if __name__ == "__main__":
    ...
    if args.video:
        ...
        # Create an array with values from 0.1 to 1 with a step of 0.1
        styleCount = len(list(exstyles.keys()))
        styleids = np.arange(4, styleCount - 1, 1, dtype=int)
        values = np.arange(0.0, 1.01, 0.01)
        color_transfer_states = [ False, True ]
        # iterate through values
        for styleid in styleids:
            stylename = list(exstyles.keys())[styleid]
            
            exstyle = torch.tensor(exstyles[stylename]).to(device)
            with torch.no_grad():  
                exstyle = vtoonify.zplus2wplus(exstyle)
            # Save exstyle to file
            v = 0
            for color_transfer_state in color_transfer_states:
                for value in values:
                    value = round(value,2)
                    print(f"Processing image with value {value}/1.0, style {styleid}/{styleCount}, color_transfer_state {color_transfer_state}")
                    colorState = 'c' if color_transfer_state else 'nc'
                    savename = os.path.join(args.output_path, basename + f'_vtoonify_{str(round(value * 100)).zfill(4)}_{styleid}_{colorState}_' +  args.backbone[0] + '.jpg')
                    ...
                    with torch.no_grad():
                        ...
                        y_tilde = vtoonify(inputs, s_w.repeat(inputs.size(0), 1, 1), d_s = value)        
                        # y_tilde = vtoonify(inputs, s_w.repeat(inputs.size(0), 1, 1), d_s = args.style_degree)        
                        y_tilde = torch.clamp(y_tilde, -1, 1)

                    cv2.imwrite(cropname, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
                    save_image(y_tilde[0].cpu(), savename)
                filePattern = os.path.join(args.output_path, basename + f'_vtoonify_%04d_{styleid}_{colorState}_' +  args.backbone[0] + '.jpg')
                imageSequencetoVideo(filePattern, os.path.join(args.output_path, basename + f'_vtoonify_{styleid}_{colorState}_' +  args.backbone[0] + '.mp4'), 12)
